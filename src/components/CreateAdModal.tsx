...
export function CreateAdModal() {
  ...
  useEffect(() => {
    axios('http://server.pessoal.ws:3333/games')
      .then(response => {
  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    try {
      await axios.post(`http://server.pessoal.ws:3333/games/${data.game}/ads`, {
        "name": data.name,
          ...
          <div className='flex gap-6'>
            <div className='flex flex-col gap-2'>
              ...
              <ToggleGroup.Root
                ...
                <ToggleGroup.Item
                  value='0'
                  className={`w-8 h8 rounded  ${weekDays.includes('0') ? 'bg-violet-600' : 'bg-zinc-900'}`} title='Domingo'
                >
                  D
                </ToggleGroup.Item >
                <ToggleGroup.Item
                  value='1'
                  className={`w-8 h8 rounded  ${weekDays.includes('1') ? 'bg-violet-500' : 'bg-zinc-900'}`} title='Segunda'
                >
                  S
                </ToggleGroup.Item >
                <ToggleGroup.Item
                  value='2'
                  className={`w-8 h8 rounded  ${weekDays.includes('2') ? 'bg-violet-500' : 'bg-zinc-900'}`} title='Terça'
                >
                  T
                </ToggleGroup.Item >
                <ToggleGroup.Item
                  value='3'
                  className={`w-8 h8 rounded  ${weekDays.includes('3') ? 'bg-violet-500' : 'bg-zinc-900'}`} title='Quarta'
                >
                  Q
                </ToggleGroup.Item >
                <ToggleGroup.Item
                  value='4'
                  className={`w-8 h8 rounded  ${weekDays.includes('4') ? 'bg-violet-500' : 'bg-zinc-900'}`} title='Quinta'
                >
                  Q
                </ToggleGroup.Item >
                <ToggleGroup.Item
                  value='5'
                  className={`w-8 h8 rounded  ${weekDays.includes('5') ? 'bg-violet-500' : 'bg-zinc-900'}`} title='Sexta'
                >
                  S
                </ToggleGroup.Item >
                <ToggleGroup.Item
                  value='5'
                  className={`w-8 h8 rounded  ${weekDays.includes('6') ? 'bg-violet-500' : 'bg-zinc-900'}`} title='Sábado'
                >
                  S
                </ToggleGroup.Item >
              </ToggleGroup.Root>
